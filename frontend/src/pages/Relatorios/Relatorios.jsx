...
const Relatorios = () => {
  ...
  const reportsData = [
    {
      ...
      descricao:
        "Lista todas as turmas do sistema com respectivo curso, período e número de alunos.",
      categoria: "Académico",
      formato: "PDF",
      requiresConfig: true,
    },
    {
      ...
      descricao:
        "Gera a pauta nominal completa de uma turma específica com número de processo e status.",
      categoria: "Académico",
      formato: "PDF",
      requiresConfig: true,
    },

    {
      ...
      descricao:
        "Lista de todos os candidatos que realizaram inscrição no portal público.",
    ...
    },
    {
      ...
      descricao:
        "Histórico de anos lectivos registrados, datas de vigência e estado actual.",
      categoria: "Administrativo",
      formato: "PDF",
    ...
    },
    {
      ...
      descricao:
        "Visualização da distribuição de alunos por blocos e salas de aula.",
    ...
    },
    {
      ...
      descricao:
        "Controle de vagas oferecidas, ocupadas e disponíveis por curso.",
      categoria: "Administrativo",
      formato: "PDF",
      requiresConfig: true,
    },
  ...
  const finalReports = reportsData.filter(
  ...
  const handleGenerateReport = async (reportId, config = {}) => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute("download", `${reportId}_${new Date().getTime()}.pdf`);
      ...
      alert(
        "Não foi possível gerar este relatório no momento. Verifique se os dados estão preenchidos corretamente.",
      );
    } finally {
      setGenerating(null);
    }
  ...
  const filteredReports = finalReports.filter(
    (report) =>
    ...
    <div className="page-container relatorios-page">
      <header className="page-header">
        <div className="page-header-content">
          <div>
            ...
            <p>
              Gere documentos oficiais, listas nominais e analise dados
              institucionais.
            </p>
          </div>
        </div>
      ...
      <div className="table-card" style={{ marginTop: "24px" }}>
        ...
        <div className="search-filter-row">
          ...
          >
            {["Todos", "Académico", "Administrativo", "Infraestrutura"].map(
              (cat) => (
                <button
                  key={cat}
                  onClick={() => {
            ...
            <tbody>
              {currentItems.map((report) => (
                ...
                >
                  ...
                  <td>
                    <span
                      className={`status-badge ${report.categoria.toLowerCase()}`}
                      style={{
                        background: "#f8fafc",
            ...
            <div className="modal-body" style={{ padding: "24px" }}>
              ...
              >
                ...
                <p style={{ margin: 0, fontSize: "13px", color: "#64748b" }}>
                  {selectedReportTemplate?.descricao}
                </p>
              </div>
              ...
              {selectedReportTemplate?.id === "relatorio_turmas" && (
                <div className="form-group" style={{ marginBottom: "16px" }}>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setConfigData({ ...configData, ano_id: e.target.value })
                    }
                  >
                    <option value="all">Todos os anos</option>
                    {auxData.anos.map((a) => (
                      <option key={a.id_ano} value={a.id_ano}>
                        {a.nome}
                      </option>
                    ))}
                  </select>
              ...
              {selectedReportTemplate?.id === "alunos_por_turma" && (
                <div className="form-group" style={{ marginBottom: "16px" }}>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setConfigData({ ...configData, turma_id: e.target.value })
                    }
                  >
                    <option value="">Escolha uma turma...</option>
                    {auxData.turmas.map((t) => (
                      <option key={t.id_turma} value={t.id_turma}>
                        {t.codigo_turma} - {t.curso_nome}
                      </option>
                    ))}
                  </select>
                </div>
              ...
              {(selectedReportTemplate?.id === "inscritos_por_ano" ||
                selectedReportTemplate?.id === "relatorio_vagas") && (
                <div className="form-group" style={{ marginBottom: "16px" }}>
                  <label
                    style={{
                      display: "block",
              ...
              <button
                ...
                onClick={() =>
                  handleGenerateReport(selectedReportTemplate.id, configData)
                }
              >
                {generating === selectedReportTemplate?.id ? (
