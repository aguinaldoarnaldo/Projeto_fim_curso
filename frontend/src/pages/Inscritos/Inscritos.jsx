...
const ListaEsperaPanel = ({
  ...
  useEffect(() => {
    if (!showAddModal || candidateSearch.length < 2) {
  ...
  const handleRemove = async (id, nome) => {
    if (
      !window.confirm(
        `Remover "${nome}" da lista de espera? O status será revertido para NAO_CLASSIFICADO.`,
      )
    )
      return;
    try {
  ...
  const currentData = filteredLista.slice(
    ...
    <div className="le-panel">
      ...
      <div className="le-panel-stats">
        <div className="le-pstat le-pstat-total">
          <Users size={18} />
          <div>
            <span className="le-pstat-val">{stats.total}</span>
            <span className="le-pstat-lbl">Na Lista</span>
          </div>
        </div>
        <div className="le-pstat le-pstat-waiting">
          <Clock size={18} />
          <div>
            <span className="le-pstat-val">{stats.aguardando}</span>
            <span className="le-pstat-lbl">Aguardando</span>
          </div>
        </div>
        <div className="le-pstat le-pstat-called">
          <CheckCircle size={18} />
          <div>
            <span className="le-pstat-val">{stats.chamados}</span>
            <span className="le-pstat-lbl">Chamados</span>
          </div>
        </div>
        <div style={{ flex: 1 }} />
      ...
      <div className="le-info-bar">
        <Info size={14} />
        <span>
          Candidatos nesta lista têm status <strong>LISTA DE ESPERA</strong>.
          Quando houver vaga disponível, use o botão <strong>Matricular</strong>{" "}
          para matriculá-los directamente.
        </span>
      </div>
        ...
        <table className="data-table le-table">
          <thead>
            <tr>
              ...
              <th
                onClick={() => requestSort("candidato_nome")}
              ...
              <th
                onClick={() => requestSort("data_entrada")}
                style={{ cursor: "pointer" }}
              >
          ...
          >
            <div
              ...
              style={{
                ...
                boxShadow:
                  "0 25px 60px -12px rgba(0,0,0,0.35), 0 0 0 1px rgba(0,0,0,0.06)",
                overflow: "hidden",
            ...
            >
              ...
              >
                ...
                >
                  <div className="le-modal-icon-wrap">
                    <UserPlus size={20} />
                  </div>
                  <div>
                    <h3
                ...
                <button onClick={resetModal} className="le-close-btn">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="le-steps-bar">
                <div
                  className={`le-step-indicator ${addStep >= 1 ? "active" : ""} ${addStep > 1 ? "done" : ""}`}
                >
                  <span className="le-step-num">1</span> Candidato
                </div>
                <div className="le-step-connector" />
                <div
                  className={`le-step-indicator ${addStep >= 2 ? "active" : ""}`}
                >
                ...
                {addStep === 1 && (
                  ...
                  >
                    <div className="le-info-note">
                      <Info size={14} />
                      Apenas candidatos com status{" "}
                      <strong>NAO_CLASSIFICADO</strong>,{" "}
                      <strong>AUSENTE</strong> ou <strong>INSCRITO</strong>{" "}
                  ...
                  >
                    ...
                    <div className="le-selected-box">
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        >
                          {selectedCandidate.numero_inscricao} •{" "}
                          {selectedCandidate.curso1_nome || "Sem curso"}
                        </div>
                      </div>
                      <button
                        onClick={() => {
...
const Inscritos = () => {
  ...
  const fetchCandidatesData = async () => {
    ...
    return data.map((c) => ({
      id: (
        c.numero_inscricao || `INS-${c.id_candidato || "UNKNOWN"}`
      ).toString(),
      ...
      anoInscricao:
        c.ano_lectivo_nome ||
        (c.criado_em ? new Date(c.criado_em).getFullYear().toString() : "2026"),
  ...
  useEffect(() => {
    api
      .get("cursos/")
      .then((r) => setCursosDisponiveis(r.data.results || r.data || []))
      .catch(() => {});
    api
      .get("anos-lectivos/?all=true")
      .then((r) => setAnosDisponiveis(r.data.results || r.data || []))
      .catch(() => {});
  }, []);
  ...
  const filteredInscritos = useMemo(() => {
    ...
    items = items.filter((i) => {
      ...
      const matchesStatus =
        !filters.status ||
        (i.status || "").toUpperCase() === filters.status.toUpperCase();
      const matchesCurso = !filters.curso || i.curso1 === filters.curso;
      const matchesRUP =
        !filters.status_rup ||
        (i.rupe?.status_rup || "").toUpperCase() ===
          filters.status_rup.toUpperCase();
      return (
        matchesSearch &&
        matchesAno &&
        matchesStatus &&
  ...
  const currentData = filteredInscritos.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage,
  );
  ...
  const handleSubmitEvaluation = async () => {
    if (!examGrade || isNaN(examGrade) || examGrade < 0 || examGrade > 20) {
      alert("Nota válida: 0-20.");
      return;
    }
    try {
      const grade = parseFloat(examGrade);
      const response = await api.post(
        `candidaturas/${candidateToEvaluate.real_id}/avaliar/`,
        { nota: grade },
      );
  ...
  const handleSaveEdit = async (data) => {
    ...
    try {
      ...
      if (data.ano_conclusao)
        formData.append("ano_conclusao", toInt(data.ano_conclusao));
      if (data.media_final)
        formData.append("media_final", toDecimal(data.media_final));
      ...
      if (data.notaExame !== "")
        formData.append("nota_exame", toDecimal(data.notaExame));
      ...
      alert(parseApiError(error, "Erro ao salvar alterações."));
    }
  };
  ...
  const handleConfirmPayment = async (candidato) => {
    ...
    if (
      !window.confirm(
        `Confirmar pagamento do RUPE ${candidato.rupe.codigo_rup}?`,
      )
    )
      return;
    try {
  ...
  const handleDistributeExams = async () => {
    ...
    if (
      window.confirm(
        "Distribui candidatos do ano lectivo ativo por salas. Continuar?",
      )
    ) {
      try {
        setIsProcessingExams(true);
        const res = await api.post(
          "candidaturas/distribuir_exames/",
          examConfig,
        );
  ...
  return (
    <div className="page-container inscritos-page">
      ...
      <CandidateDetailModal
        ...
        onGenerateRUP={() => setRupGenerated(true)}
        onRefresh={refresh}
